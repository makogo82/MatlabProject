...
shots_endtime = [ 1.29  0.904 1.148  0.9415 0.9855 1.195 1.22   1.12    0.857 ];
...
Kpe = 0.20/150; % the nonlinear "proportional" gain Kp*(1+Kpe*error)
        ...
        switch(qPIDH)
            ...
            case 0
                ...
                if( ((xfilterState >= errorDeazoneH && abs(d_error)<= sigmaHpid2) ||...
                   ...
                   && switches_counter >= avoid_chattering_PIDH_OFF ) 
                   qOFF = 0;
...
%                 end
                ...
                if(dd_error <= -gammaHpid2  && switches_counter >= avoid_chattering_PIDH_OFF)
                ...
                if(dd_error >= -gammaHpid2  && d_error >= sigmaHpid1 && switches_counter >= avoid_chattering_PIDH_OFF)
                    qOFF = 0;
                end
...
%               end
    ...
    if(qOFF==1)
        ...
        theta_gains = 1.0*min(1.0,abs(dd_error)/(1.5*gammaHpid1)) + (1-min(1.0,abs(dd_error)/(1.5*gammaHpid1)))*theta_gain_0;
    else
        theta_gains = 1.0;
    end
