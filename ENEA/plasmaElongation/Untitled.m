...
if(simula_mymodel==1)
    ...
    t1 = c_2*data.IPLmis'.*data.Hmis'.*(1./(c_3-(data.dez+b_pl)) - 1./(-c_3-(data.dez+b_pl)) )';
    
    gamma = 1;
    
    t1H = c_2H*data.IPLmis'.*data.Hmis'.*(1./(c_3-(gamma*data.dez+b_pl)) - 1./(-c_3-(gamma*data.dez+b_pl)) )';    
    t1L = c_2L*data.IPLmis'.*data.Hmis'.*(1./(c_3-(data.dez+b_pl)) - 1./(-c_3-(data.dez+b_pl)) )';
    ...
    c_5 = 0.4;
    b_pl = 0;
    t2 = -c_4*data.Fmis'.*data.IPLmis'.*(4*( 1./(c_5-(data.dez+b_pl)) + 1./(-c_5-(data.dez+b_pl)))).^2';
    ...
    ylim([-3000, 3000]);
    title(num2str(shot))
    legend('t1','t2','t3','t1H','t1L','dd_dez','location','SouthWest');
    break
    
    
    ax(1) = subplot(2,1,1);
    plot(mytime, t1,'r',mytime, t2,'b',...
         mytime,t1+t2+t3,'g',...
         mytime,t1H+t2+t3,'g-.',...
         mytime,t1L+t2+t3,'g--',...
         mytime,t3,'c--',...
         mytime,dd_error_memo*(((1.5e-4) * (-1.0))),'k','LineWidth',2);
    grid on
    ylim([-3000, 3000]);
    title(num2str(shot))
    legend('c1','c2','dd_dez_{iden}','dd_dez_{iden}H','dd_dez_{iden}L','dd_dez_{exp}','-','location','SouthWest');
