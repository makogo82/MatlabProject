...
bool IreRef::ObjectLoadSetup(ConfigurationDataBase &cdbData, StreamInterface * err) {
    ...
    cdb.ReadFloat( 	ireRef_beta      				,"REREF_beta", 						0.9);//            % exponential decay rate
    cdb.ReadFloat( 	ireRef_deltaTdes 				,"REREF_deltaTdes", 				0.2);//            % 300ms
    cdb.ReadInt32( 	ireRef_enableMultiplePlateau 	,"REREF_enableMultiplePlateau", 	1);//  % enable multiple plateau
    //shift the straight line of the new ipref
    cdb.ReadFloat( 	ireRef_threshold_time_newiref 	,"REREF_threshold_time_newiref", 	0.02);//    %200ms
    cdb.ReadFloat( 	ireRef_percent_upgain_newiref 	,"REREF_percent_upgain_newiref",	 0.1);//    % if the (ipl current - newIreftpl) >= 0 then  increase Iref_linear of 5%
    cdb.ReadFloat( 	ireRef_enableUpdateRefUp 		,"REREF_enableUpdateRefUp",			 0);//      %old version
...
float IreRef::new_re_ref(int32 time, float ipl, float ipl_ref, float vloop, ControllerGAMOutputStructure* outputData){
    // execution order
...
//             if (iref_output <= 0 || ipl <= cq_minIPL )
//                 iref_output = 0;
            ...
            //iref_output = iref_output*ipl_sign;
